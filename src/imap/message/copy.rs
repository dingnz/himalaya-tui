...
pub struct ImapMessageCopyHandler {
    pub mailbox: String,
    pub id: String,
    pub target: String,
}

impl ImapMessageCopyHandler {
    pub fn execute(self, session: &mut ImapSession) -> Result<()> {
        ...
        loop {
            match coroutine.resume(arg.take()) {
                ImapCopyResult::Io { io } => arg = Some(handle(&mut session.stream, io)?),
                ImapCopyResult::Ok { context, .. } => {
                    session.context = context;
                    break;
                }
                ImapCopyResult::Err { context, err } => {
                    session.context = context;
                    bail!(err);
                }
            }
